        ...
        if UserInput == "1":
            ...
            if UserInput == "1":
                print(f"\nИван: Да, к сожалению так, {Player.name}"
                      f"\n Но пока ещё все в городе, сбегай к друзьям, поговори. А мы пока тут посидим")
            ...
            print("\nИван: Птичье молоко? Конечно давай...Ном-Ном-Ном(кушает конфеты*)"
                  "\nСтепан: Спасибо, но я пожалуй откажусь")
            In_Pizza_Dialog()
        else:
            print("Пожалуйста введите цифры")
...
def Cinema_Dialog():
    ...
    while UserInput not in options:
        ...
        if UserInput == "1":
            ...
            print("\nКостя: Не очень их люблю, но спасибо за предложение")
            Cinema()
        else:
            print("Пожалуйста введите цифры")
